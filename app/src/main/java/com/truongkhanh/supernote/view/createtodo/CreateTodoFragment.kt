...
class CreateTodoFragment : BaseFragment() {
    ...
    private var isDoneLoadData = false
    ...
    private fun getDataFromBundle() {
        arguments?.let { bundle ->
            ...
            bundle.getParcelable<Todo>(TODO_BUNDLE)?.let {todo ->
                createTodoViewModel.isUpdate.value = true
                updateInfo(todo)
            }
        }
    }

    private fun updateInfo(todo: Todo) {
        createTodoViewModel.apply {
            this.id.value = todo.id
            this.alert.value = todo.alertType
            this.checkList.value = todo.checkList?.checkItemsFromString()
            ...
            this.startCalendar.value = todo.startDate.toCalendar()
            this.endCalendar.value = todo.endDate.toCalendar()
            this.scheduleItems.value = todo.schedule?.scheduleItemsFromString()
        }
    }
    ...
    private fun bindingViewModel() {
        ...
        createTodoViewModel.startCalendar.observe(this, Observer { startCalendar ->
            ...
            if (isDoneLoadData || !createTodoViewModel.isUpdate.value!!)
                getScheduleItems()?.let {
                    createTodoViewModel.scheduleItems.value = it
                }
        })
        ...
        createTodoViewModel.priority.observe(this, Observer {priority ->
    ...
    private fun clickListener() {
        ...
        RxView.clicks(btnSave)
            .throttleFirst(THROTTLE_TIME, TimeUnit.MILLISECONDS)
            .subscribe {
                createTodoViewModel.isUpdate.value?.let {isUpdate ->
                    if (isUpdate)
                        createTodoViewModel.updateTodo()
                    else
