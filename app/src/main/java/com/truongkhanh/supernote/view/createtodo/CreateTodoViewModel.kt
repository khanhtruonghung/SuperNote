...
class CreateTodoViewModel(private val context: Context) : ViewModel() {
    ...
    val isUpdate = MutableLiveData<Boolean>().apply {
        postValue(false)
    }
    ...
    fun updateTodo() {
        Todo(
            ...
            todo.notificationRequestID = buildNotificationWorker(todo)
            todoRepository.update(todo)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe {
                    if (!tagList.value.isNullOrEmpty()) {
                        deleteTodoTagList(todo)
                    } else {
    ...
    private fun deleteTodoTagList(todo: Todo) {
        todoTagListRepository.deleteAll(todo.id)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeOn(Schedulers.io())
            .subscribe {
    ...
    private fun buildNotificationWorker(todo: Todo): String {
        ...
        if (todo.alertType != AlertPickerDialogFragment.NO_ALERT) {
            val alertType = todo.alertType
            todo.schedule?.scheduleItemsFromString()?.forEach{ item ->
                ...
                val notificationRequest = OneTimeWorkRequestBuilder<NotificationWorker>()
                    .setInputData(data)
                    .setInitialDelay(calculateDelay2(item.date, alertType), TimeUnit.MILLISECONDS)
                    .build()
                WorkManager.getInstance(context)
                    .enqueue(notificationRequest)
                notificationList.add(NotificationItem(notificationRequest.id.toString()))
            }
        }
        return notificationList.notificationItemsToString()
    }
    ...
    private fun calculateDelay2(startDate: Long, alertType: Int): Long {
